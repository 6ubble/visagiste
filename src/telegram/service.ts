...
import { SERVICE_TYPES } from '../components/OrderForm/constants.ts'
...
export class TelegramService {
  ...
  private static getServiceTypeName(serviceTypeId: string): string {
    const service = SERVICE_TYPES.find((s: { id: string; name: string }) => s.id === serviceTypeId)
  ...
  static async sendOrder(orderData: OrderFormData): Promise<{ success: boolean; message: string }> {
    try {
      ...
      return {
        success: true,
        message: 'Заявка успешно отправлена! Я свяжусь с вами в ближайшее время для уточнения деталей.'
      }
