import { useHeroSlider } from '../../shared/hooks/useHeroSlider.ts'
import { useHeroImages } from '../../shared/hooks/useImageQuery.ts'
import { BUTTON_LABELS, HERO_TEXT } from './constants.ts'

function Hero(): React.JSX.Element {
  const { currentSlide, slides, goToSlide } = useHeroSlider()
  
  // REACT QUERY магия для изображений
  const { loadedImages, isFirstImageLoaded, areAllImagesLoaded } = useHeroImages(slides)
  ...
  return (
    <section id="home" className="relative bg-gradient-to-br from-gray-900 via-black to-gray-800 overflow-hidden h-screen w-full min-h-screen">
      ...
      {/* Фоновые изображения с React Query кэшированием */}
      {slides.map((slide, index) => (
        ...
        >
          ...
          {loadedImages.has(index) ? (
            <img 
              src={slide.image}
              alt={`${HERO_TEXT.title} - слайд ${index + 1}`}
      ...
      {/* Контент - показываем только когда первое изображение загружено */}
      {isFirstImageLoaded && (
      ...
      {/* Индикаторы слайдов с состоянием загрузки */}
            ...
            className={`w-4 h-4 md:w-5 md:h-5 rounded-full transition-all duration-300 ease-in-out transform ${
              index === currentSlide 
                ...
                : loadedImages.has(index)
                  ...
                  : 'bg-gray-600/30 animate-pulse' // Пульсация для загружающихся
            }`}
            disabled={!loadedImages.has(index)}
            title={loadedImages.has(index) ? `Слайд ${index + 1}` : 'Загружается...'}
          />
        ))}
      </div>
      ...
      {!areAllImagesLoaded && (
        <div className="absolute top-4 right-4 z-40">
          <div className="bg-black/50 backdrop-blur-sm rounded-full px-4 py-2 flex items-center space-x-2">
            ...
            <span className="text-white text-sm">Загружаем изображения...</span>
          </div>
        </div>
      )}
    </section>
